...
export const ImportView: React.FC<ImportViewProps> = ({ 
  ...
  const renderUpload = () => (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8 animate-in fade-in duration-500">
      <div 
        ...
        `}
      >
        <div className="w-16 h-16 sm:w-24 sm:h-24 bg-slate-200/50 dark:bg-white/5 border border-slate-300/50 dark:border-white/10 rounded-[1.2rem] sm:rounded-3xl flex items-center justify-center mb-6 sm:mb-8 shadow-inner ring-1 ring-white/10 group">
          <Upload className={`w-8 h-8 sm:w-10 sm:h-10 ${drag ? "text-primary" : "text-slate-500 dark:text-white/40"} transition-all group-hover:scale-110 group-hover:text-primary group-hover:opacity-100`} />
        </div>
        
        <h3 className="text-2xl sm:text-3xl font-black text-text-main mobile-heading mb-3 tracking-tighter">Import Transactions</h3>
        <p className="text-sm sm:text-base font-medium text-slate-600 dark:text-slate-300 mb-8 sm:mb-10 max-w-sm mx-auto">Upload your CSV or Excel bank statement. We'll help you map the columns to Vylos.</p>
      ...
      <div className="flex flex-col gap-6">
        <div className="vylos-glass-readable p-6 sm:p-8 !rounded-[1.5rem] sm:!rounded-[2.5rem]">
          ...
          <ul className="space-y-3">
            {["Standard CSV", "Excel Sheets (.xlsx)", "Standard Bank Formats"].map(f => (
              <li key={f} className="flex items-center gap-2 text-xs font-bold text-slate-700 dark:text-slate-200">
        ...
        <div className="vylos-glass-readable p-6 sm:p-8 !rounded-[1.5rem] sm:!rounded-[2.5rem] flex-1">
          <h4 className="text-[11px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest mb-4">Pro Tip</h4>
  ...
  const renderMapping = () => (
    <div className="vylos-glass-readable !rounded-[1.5rem] sm:!rounded-[3rem] p-5 sm:p-10 animate-in slide-in-from-bottom-8 duration-500">
      <div className="flex items-center justify-between mb-8 sm:mb-10">
        <div>
          <h3 className="text-xl sm:text-2xl font-black text-text-main mobile-heading tracking-tighter">Map your data</h3>
          <p className="text-xs sm:text-sm font-medium text-slate-600 dark:text-slate-300">Tell us which columns contain your transaction details.</p>
        </div>
        <button 
          onClick={() => setStage("upload")}
          className="p-2 sm:p-3 text-slate-500 dark:text-slate-400 hover:text-red-500 transition-colors"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        <div className="flex flex-col gap-6">
          <div className="space-y-4">
            <label className="text-[10px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest block pl-2">Date Column</label>
            <select 
              value={mapping.date}
              onChange={e => setMapping({ ...mapping, date: e.target.value })}
              className="w-full vylos-glass-input rounded-2xl p-4 text-sm font-bold focus:ring-2 focus:ring-primary outline-none appearance-none"
            >
              <option value="" className="text-slate-900 dark:text-white bg-white dark:bg-slate-900">Select Column...</option>
              {rawData?.headers.map(h => <option key={h} value={h} className="text-slate-900 dark:text-white bg-white dark:bg-slate-900">{h}</option>)}
            </select>
          </div>

          <div className="space-y-4">
            <label className="text-[10px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest block pl-2">Description / Merchant</label>
            <select 
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
               <label className="text-[10px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest block pl-2">Amount Strategy</label>
               <button 
            ...
            {mapping.amount !== undefined ? (
              <select 
                ...
                className="w-full vylos-glass-input rounded-2xl p-4 text-sm font-bold focus:ring-2 focus:ring-primary outline-none appearance-none"
              >
                <option value="" className="text-slate-900 dark:text-white bg-white dark:bg-slate-900">Select Single Amount Column...</option>
                {rawData?.headers.map(h => <option key={h} value={h} className="text-slate-900 dark:text-white bg-white dark:bg-slate-900">{h}</option>)}
              </select>
            ) : (
              <div className="grid grid-cols-2 gap-4">
                <select 
                  ...
                  className="vylos-glass-input rounded-2xl p-4 text-sm font-bold outline-none"
                >
                  <option value="" className="text-slate-900 dark:text-white bg-white dark:bg-slate-900">Debit...</option>
                  {rawData?.headers.map(h => <option key={h} value={h} className="text-slate-900 dark:text-white bg-white dark:bg-slate-900">{h}</option>)}
                </select>
                <select 
                  ...
                  className="vylos-glass-input rounded-2xl p-4 text-sm font-bold outline-none"
                >
                  <option value="" className="text-slate-900 dark:text-white bg-white dark:bg-slate-900">Credit...</option>
                  {rawData?.headers.map(h => <option key={h} value={h} className="text-slate-900 dark:text-white bg-white dark:bg-slate-900">{h}</option>)}
                </select>
              </div>
            )}
          ...
          <div className="space-y-4">
            <label className="text-[10px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest block pl-2">Category (Optional)</label>
            <select 
              value={mapping.category || ""}
              onChange={e => setMapping({ ...mapping, category: e.target.value })}
              className="w-full vylos-glass-input rounded-2xl p-4 text-sm font-bold focus:ring-2 focus:ring-primary outline-none appearance-none"
            >
              <option value="" className="text-slate-900 dark:text-white bg-white dark:bg-slate-900">Auto-categorize (None)</option>
              {rawData?.headers.map(h => <option key={h} value={h} className="text-slate-900 dark:text-white bg-white dark:bg-slate-900">{h}</option>)}
            </select>
          </div>
        </div>

        <div className="bg-slate-100/50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-[2rem] p-5 sm:p-8 flex flex-col">
          <div className="flex items-center gap-3 mb-6">
            <Table size={18} className="text-slate-500 dark:text-slate-400" />
            <h4 className="text-[11px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest">Data Preview (First 5 rows)</h4>
          </div>
              ...
              <thead>
                <tr>
                  {rawData?.headers.slice(0, 4).map(h => (
                    <th key={h} className="text-[9px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest pb-4 pr-4">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {rawData?.rows.slice(0, 5).map((row, i) => (
                  <tr key={i}>
                    {rawData?.headers.slice(0, 4).map(h => (
                      <td key={h} className="text-[10px] font-bold text-slate-700 dark:text-slate-200 py-2 pr-4 truncate max-w-[100px]">{String(row[h])}</td>
                    ))}
                  </tr>
                ))}
              </tbody>
    ...
    <div className="vylos-glass-readable !rounded-[1.5rem] sm:!rounded-[3rem] shadow-2xl flex flex-col max-h-[calc(100vh-200px)] overflow-hidden animate-in fade-in zoom-in-95 duration-500">
      <div className="p-5 sm:p-10 border-b border-slate-200 dark:border-white/10 flex flex-col md:flex-row gap-6 md:items-center justify-between bg-white/5 dark:bg-white/5 backdrop-blur-xl shrink-0">
        <div className="flex flex-col sm:flex-row sm:items-center gap-6 sm:gap-10">
          <div>
            <h3 className="text-xl sm:text-2xl font-black text-text-main mobile-heading tracking-tighter">Review synchronization</h3>
            <p className="text-[11px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-[0.2em] mt-1">{(importPreview || []).length} records staging for commit</p>
          </div>

          <div className="flex items-center gap-6 border-l border-slate-200 dark:border-white/10 pl-6 sm:pl-10">
            <div className="flex flex-col">
              <span className="text-[10px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest">Duplicates Skipped</span>
              <span className="text-lg font-black text-amber-500">{(importPreview || []).filter(tx => tx.isDuplicate).length}</span>
            </div>
            <div className="flex flex-col">
              <span className="text-[10px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest">Net New</span>
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          <button 
            ...
            className="flex-1 md:flex-initial px-6 sm:px-8 py-3.5 sm:py-4 rounded-[1.2rem] text-sm font-black text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white transition-colors border border-slate-300 dark:border-white/15 hover:bg-slate-200/50 dark:hover:bg-white/5"
          >
            Discard
          </button>
          <button 
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="flex flex-col">
          {(importPreview || []).map((tx, i) => (
            <div key={i} className={`flex items-center gap-3 sm:gap-6 px-4 sm:px-10 py-4 sm:py-6 border-b border-slate-100 dark:border-white/5 transition-colors group ${tx.isDuplicate ? 'opacity-65 dark:opacity-40 bg-slate-100/50 dark:bg-slate-800/30' : 'hover:bg-primary/[0.02]'}`}>
              <div 
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <div className={`text-[13px] sm:text-base font-black truncate tracking-tight ${tx.isDuplicate ? 'text-slate-500 dark:text-slate-400 line-through' : 'mobile-heading'}`}>{tx.desc}</div>
                ...
                <div className="text-[10px] sm:text-[11px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest flex items-center gap-1.5 sm:gap-3 mt-1">
                  <span>{tx.date}</span>
              ...
              <div className="flex items-center gap-3 sm:gap-8 shrink-0 ml-3">
                <div className={`text-[13px] sm:text-lg font-black tracking-tighter ${tx.isDuplicate ? 'text-slate-500 dark:text-slate-400' : (tx.amount > 0 ? "text-primary" : "text-red-500")}`}>
                  {formatCurrency(tx.amount)}
                </div>
                <button 
                  onClick={() => setImportPreview((importPreview || []).filter((_, idx) => idx !== i))}
                  className="p-1.5 sm:p-2 text-slate-500 dark:text-slate-400 hover:text-red-500 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-all shrink-0"
                >
  ...
  return (
    <ViewContainer className={`flex flex-col gap-6 sm:gap-10 pt-4 ${isMobile ? 'pb-24 max-w-md mx-auto px-1' : ''}`}>
      {isMobile && (
        <MobilePageHeader 
