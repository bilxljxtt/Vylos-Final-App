...
export function ExportTransactionsModal({ isOpen, onClose, data }: ExportTransactionsModalProps) {
  ...
  const handleExport = async () => {
    ...
    try {
      // 1. Fetch data
      let transactionsToExport = data;
      
      // If no filtered data provided, we fetch EVERYTHING from the DB
      if (!transactionsToExport) {
      ...
      if (format === "csv") {
        ...
        autoTable(doc, {
          ...
          didDrawPage: (data) => {
            ...
            doc.text(`Page ${data.pageNumber} of ${totalPages}`, 105, 285, { align: "center" });
        ...
        <div className="relative vylos-modal-glass w-full max-w-md rounded-[2.5rem] shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-500">
          ...
          <div className="p-8 space-y-8">
            ...
            <div className="p-4 rounded-2xl bg-amber-500/5 border border-amber-500/10 flex items-start gap-3">
              ...
              <div>
                <p className="text-[11px] font-black text-amber-600 uppercase tracking-widest">Exporting</p>
                <p className="text-[10px] font-medium text-text-muted leading-relaxed">
                  All {data?.length || state.transactions.length} transactions for your active account will be included in this export.
                </p>
              </div>
            </div>
          </div>
